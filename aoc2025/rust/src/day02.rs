...
pub fn parse_input(lines: &[String]) -> Vec<Range> {
    assert!(lines.len() == 1);
    lines[0]
        .split(",")
        .filter_map(|s| {
            ...
            Some(Range {
                start: first.parse::<i64>().ok()?,
                end: second.parse::<i64>().ok()?,
            })
        })
        .collect()
}
...
pub fn solve_part2(lines: &[String]) -> Result<i64> {
    ...
    let all_proper_divisors: HashMap<i64, Vec<i64>> = (2..=max_length as i64)
        ...
        .collect();
    let divisors_with_multipliers: HashMap<i64, Vec<DivisorPair>> =
        all_proper_divisors
            .iter()
                ...
                let pairs = divisors
                    ...
                    .map(|&divisor| {
                        let multiplier = (powers_of_ten[num_digits as usize]
                            - 1)
                            / (powers_of_ten[divisor as usize] - 1);
                        DivisorPair {
                            divisor,
                            multiplier,
                        }
    ...
    let invalid_sum: i64 = input
        ...
        .map(|&bounds| {
            ...
            for id in bounds.start..=bounds.end {
                ...
                for pair in &divisors_with_multipliers[&num_digits] {
                    let pattern = id
                        / powers_of_ten[(num_digits - pair.divisor) as usize];
                    let repeated = pattern * pair.multiplier;
                    if id == repeated {
                        local_sum += id;
                        break;
                    }
